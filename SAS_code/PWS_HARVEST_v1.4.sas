...
LIBNAME SASDATA BASE "O:\DSF\RTS\common\PAT\PERMITS\&PROJECT\&newYEAR";
...
Since the date appears to be fixed, we don't need to reset the datepart, we'll just make
harvedate1 = harvdate this year 12/23.  Keep the code incase it is a problem again. 
...
DATA HARVEST; 
	SET new_date (DROP =  YEAR KEYDATE MAILING KEYID);
     ...
     IF NOCATCH = 1 THEN CATCH = 'N'; 
	IF NOCATCH NE 1 THEN CATCH = 'Y';
     IF NOHRVRPT = 1 THEN HRVRPT = 'N'; 
	IF NOHRVRPT NE 1 THEN HRVRPT = 'Y';
...
data cln_har2 (rename = (harvdate1 = harvdate));
	set cln_har;
	location = location_1;
	drop location_1;
run;
...
for delimited files;
proc import 
 datafile = "O:\DSF\RTS\common\Pat\Permits\Shrimp\&newyear.\location_statarea_updated_&newyear..csv"
 out = loc_stat
 dbms = csv
 replace;
 guessingrows = max;
...
PROC EXPORT DATA = loc_stat1
    OUTFILE = 
	"O:\DSF\RTS\common\PAT\PERMITS\&PROJECT\&newYEAR\location_statarea_updated_&newyear..csv" 
	DBMS = csv REPLACE;
RUN;
...
* In 2023, the harvest file was cleaned before we exported it and changed it, add code to address that here;
data harvest_a (rename = (harvdate1 = harvdate));
	set harvest;
run;
...
this is a perfect merge and should be modified to be more of a join without
the additional outputs to onlyhv and onlystat;
...
tables harvdate * week/ out = harvdate;
run;

* Export harvest 4 to cross check weeks;
proc export data = harvest_4
	outfile = "O:\DSF\RTS\common\Pat\Permits\Shrimp\2023\harvest_4.csv"
	dbms = csv
	replace;
run;

proc export data = harvdate
	outfile = "O:\DSF\RTS\common\Pat\Permits\Shrimp\2023\harvdate.csv"
	dbms = csv
	replace;
run;
