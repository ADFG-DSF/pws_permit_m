...
DATA ALLPERMITS; MERGE SASDATA.SHRIMP_HARVEST (IN=INH) SASDATA.ISSUED (IN=INP); BY PERMIT;  
...
PROC PRINT DATA = ALLPERMITS (OBS = 15);
...
DATA TOTAL_ISSUED; SET SASDATA.TOTAL_ISSUED;
     RENAME N =NHAT;
     VAR_NHAT = 0;
...
PROC PRINT DATA = BHH_D; TITLE2 "ARRAYS FROM DATA SET BHH_D"; RUN;
...
DATA BHH; SET BHH (DROP = _TYPE_ _FREQ_ BHH2 SE2);
RUN;
