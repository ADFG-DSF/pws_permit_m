...
PROC SORT DATA=SASDATA.SHRIMP_HARVEST; BY PERMIT; RUN;  
PROC SORT DATA=SASDATA.ISSUED; BY PERMIT; RUN;

PROC freq data = sasdata.shrimp_harvest; tables mailing; run;
proc freq data = sasdata.issued; tables mailing; run;
proc freq data = sasdata.pwshvc2021; tables mailing; run;

DATA ALLPERMITS; 
	MERGE SASDATA.SHRIMP_HARVEST (IN=INH) SASDATA.ISSUED (IN=INP); 
	BY PERMIT;  
...
proc freq data = allpermits; tables mailing; run;
...
DATA HARVESTED; 
	SET RETURNED;
     ...
     FORMAT FISHERY 1.0;
     KEEP PERMIT HARVDATE &SPECIES_LIST TOTAL FISHERY COMPLIANT MAILING OLDFSHRY;
RUN;
...
proc sort data = check_1 out = check_2 nodupkey; by permit; run;
...
data est_check;
	set bhh_d;
	harv = bnh_d_fished * mean1;
	v_hat = 
	var_harv = bnh_d_fished**2 * vlhb_d + lhb_d**2 * var_bnh_d_fished - vlhb_d;
run;
...
DATA BH_X; 
	SET BH_C_1(IN=C DROP=BN_C RENAME=(_1=BHH1))
     ...
     IF dr THEN GROUP = 'NONCOMPLIANT, RESPONDED';
...
DATA BH_X; SET BH_C(IN=C DROP=BN_C RENAME=(_1=BHH1)) 
                LHB_D(IN=DR DROP= _TYPE_ LN_D_FISHED VAR_MEAN1-VAR_MEAN&MATRIX SE1-SE&MATRIX RENAME=(_1=BHH1)) 
