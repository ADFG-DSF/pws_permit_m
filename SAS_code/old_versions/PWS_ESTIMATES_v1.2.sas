...
LIBNAME SASDATA2 BASE "O:\DSF\RTS\common\PAT\PERMITS\&PROJECT\&YEAR";
...
*                              ESTIMATES                                 *;
**************************************************************************;
proc freq data = sasdata2.issued; tables status; run;

PROC FREQ DATA = sasdata2.ISSUED;
...
PROC SORT DATA=sasdata2.SHRIMP_HARVEST out = shrimp; BY PERMIT; RUN;  
PROC SORT DATA=sasdata2.ISSUED; BY PERMIT; RUN;
...
DATA ALLPERMITS; 
	MERGE shrimp_1 (IN=INH) sasdata2.ISSUED (IN=INP); 
	BY PERMIT;  
...
proc export data = harvested_1
	outfile = "I:\common\Pat\Permits\Shrimp\2021\test.csv"
	dbms = csv
	replace;
run;
...
proc sort data = sasdata2.issued out = permits; by permit; run;
