...
data both onlyloc onlyhar;
	merge m_har (in = m) loc_stat (in = l);
	by location;
	if m = 1 and l = 0 then output onlyhar;
	if m = 0 and l = 1 then output onlyloc;
	if m = 1 and l = 1 then output both;
run;
...
DATA HARVEST_1; 
	MERGE HARVEST HARVEST_CHANGE; BY HVRECID;
